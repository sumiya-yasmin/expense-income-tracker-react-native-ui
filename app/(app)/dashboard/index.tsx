...
const Index = () => {
  ...
  const CATEGORY_ICONS: Category[] =
    isActive === "EXPENSES"
      ? (EXPENSE_CATEGORY_ICONS as Category[])
      : (INCOME_CATEGORY_ICONS as Category[]);
  ...
  const renderCategoryItem = ({ item }: { item: Category }) => (
    <View
      style={{ width: itemSize, margin: itemSpacing / 2, alignItems: "center" }}
    >
      ...
      <Text className="text-[#49B58B] text-base font-semibold mt-1">
        {item.amount}
      </Text>
    </View>
  );
  ...
  const handleItemPress = (item: MOCK_DATA) => {
    router.push({
      pathname: "/(app)/dashboard/add-transaction",
      params: {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View className="flex-row justify-center py-4 px-12 gap-4">
          <TouchableOpacity
            className={`w-1/2 py-3 rounded-lg ${
              isActive === "EXPENSES" ? "bg-[#49B58B]" : "bg-[#6C6B6E]"
            }`}
          ...
          <TouchableOpacity
            className={`w-1/2 py-3 rounded-lg ${
              isActive === "INCOME" ? "bg-[#49B58B]" : "bg-[#6C6B6E]"
            }`}
        ...
        <Text className="text-xl font-bold px-5 mt-4 mb-2 text-gray-800">
          {listTitle}
        </Text>
        <FlatList
